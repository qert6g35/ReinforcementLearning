...
struct DQN{
    ...
    Policy agent = Policy(game.length, 11,3, game.actionsCount, 0.1);
    ...
    Policy train(){
        ...
        for (int i=0 ; i<episode_n ; i++){
            ...
            while(!done && steps<300){
                ...
                agent.learn({in},oldGameRepresentation);
            ...
            if(i%10 == 0 || i%10 == 1){
                ...
                cout << "[" << steps << " steps]" << endl ;//<<endl << " Szansa ma losowy krok" << eps*100.0<<endl;
            }
        }
